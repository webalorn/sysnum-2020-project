...
inline void setScreenPos(uint x, uint y) {
	send_screen_word(1, x, y, 0);
}

inline void setScreenColor(uint red, uint blue, uint green) {
...
inline void setPixelColor(uint x, uint y, uint red, uint blue, uint green) {
	send_screen_word(2, red, blue, green);
	send_screen_word(3, x, y, 0);
}

inline void drawRectangle(uint width, uint height) {
	send_screen_word(4, width, height, 0);
}

inline void drawTexture(uint id) {
	send_screen_word(5, id, 0, 0);
}

inline void startSendingTexture(uint id) {
	send_screen_word(6, id, 0, 0);
}
inline void sendTexturePixel(uint red, uint blue, uint green, uint alpha) {
	if (alpha == 0) {
